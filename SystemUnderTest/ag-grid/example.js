...
var countries_name = Object.keys(countries).map(function(key){
    return countries[key].country;
});
var games = games_data;
var booleanValues = [true, "true", false, "false"];
var firstNames = first_names_data;
var lastNames = last_names_data;
    ...
    result.push(
        {
            ...
            icon: '<img src="ag-grid/lab.svg" style="width: 14px;"/>',
            //shortcut: 'Alt + M',
            action: function () {
...
function createRowItem(row, colCount) {
    ...
    var countryData = countries[getRandomInt(countries.length - 1)];
    rowItem.country = countryData.country;
    rowItem.continent = countryData.continent;
    rowItem.language = countryData.language;

    var firstName = firstNames[getRandomInt(firstNames.length - 1)];
    var lastName = lastNames[getRandomInt(lastNames.length - 1)];
    rowItem.name = firstName + " " + lastName;

    rowItem.game = {
        name: games[getRandomInt(games.length -1)],
        bought: booleanValues[getRandomInt(booleanValues.length - 1)]
    };
...
function ratingRendererGeneral(value, forFilter) {
    ...
    for (var i = 0; i < 5; i++) {
        if (value > i) {
            result += '<img src="ag-grid/star.svg" class="star" width=12 height=12 />';
        }
    }
...
CountryFloatingFilterComponent.prototype.onParentModelChanged = function (dataModel) {
    ...
    if (dataModel) {
        ...
        toPrint.forEach(function (country) {
            country_info = getCountryByName(country)
			if (country_info.length > 0) {
				flagsHtml.push('<img class="flag" style="border: 0px; width: 15px; height: 10px; margin-left: 2px" src="' +
					country_info[0].flag_base64 + '">');
			}
			return '';
        });
...
function countryCellRenderer(params) {
    if (params.value === "" || params.value === undefined || params.value === null) {
        ...
        country_info = getCountryByName(params.value)
		if (country_info.length > 0) {
			var flag = '<img class="flag" border="0" width="15" height="10" src="' + country_info[0].flag_base64 + '">';
...
function getCountryByName(name) {
  return countries_data.filter(
      function(countries_data){ return countries_data.country == name }
  );
}
